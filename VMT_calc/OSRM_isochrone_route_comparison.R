...
function_OSRMisochrone <- function(blocks_amount_for_testing){
  ...
  for(counter_block in sspz_centroid$GEOID){
    ...
    isochrone_block <- osrmIsochrone(sspz_centroid[(sspz_centroid$GEOID == counter_block),], breaks = break_times, res = 2000, returnclass = "sf")
    ...
    # isochrone_block_intersection <- rbind(isochrone_block_intersection, missing_safegraphplaces)
    ##########
