...
```{r county commute vmt tractmode}
...
```{r county commute vmt countymode}
...
```{r compare tractmode and countymode}
load("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/county_commute_vmt_tractmode.Rdata")
...
```{r stockton commute vmt distribution 2017}
# for(year in 2011:2017){
#   
#   print(year)
#   
...
```
*Figure 8. Tracts where Stockton residents work - average commute time, in hours. Data from LODES, 2017.*
...
```{r stockton commute vmt tractmode}
...
```{r stockton commute vmt countymode}
...
```{r stockton compare tractmode and countymode}
load("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/stockton_commute_vmt_bgmode.Rdata")
...
```{r bldg dataset, residential density}
...
mapview(bg_bldg_norm_r,zcol=c("residential_density"), legend = TRUE, layer.name = "Residential density")
```
*Figure 25. Stockton residents per 1,000 sqft of residential building area, per block group.*

```{r job/sqft per block group}
bg_bldg_norm_c <-
  bg_bldg %>% 
  filter(ZONING == "C") %>% 
  left_join(
...
mapview(bg_bldg_norm_c,zcol=c("commercial_density"), legend = TRUE, layer.name = "Commercial density")
