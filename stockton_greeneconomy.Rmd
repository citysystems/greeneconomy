...
Comparison of J/ER for different counties
...
mapview(stockton_bgs_full) + mapview(stockton_boundary, alpha.regions = 0, color = "red", lwd = 4)
```
...
The following table shows the counts of jobs in these subcategories that are held by SJC residents traveling to a workplace in SJC or its top 14 contiguous neighboring counties.
...
datatable(sjc_lodes_w_top_counties_table, rownames = FALSE, options = list(pageLength = 15))
```
*Table 13. Top 15 counties where SJC residents work, including SJC. Los Angeles, San Bernardino, Orange, and San Diego Counties were removed. Data from LODES, 2017.*

The following table compares SJC with neighboring counties in terms of the percentage of jobs held by its residents that are "local", meaning within the same county.
...
```{r}
county_localjobs %>% 
  filter(County %in% c("San Joaquin","Alameda","Fresno","Stanislaus","Sacramento")) %>%
  ggplot(
    aes(
      x = Year,
      y = `Percent Jobs`,
...
```
*Table 13. Top 15 workplaces where Stockton residents work. Includes Stockton as a workplace destination separate from the rest of SJC. All other listed workplaces are counties. Los Angeles, San Bernardino, Orange, and San Diego Counties were removed. Data from LODES, 2017.*

```{r}
mapview(stockton_lodes_w_top_counties[-c(1,2),], zcol = "Percent of jobs for workers age 29 or younger", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Percent of jobs for workers age 29 or younger')
```
*Figure 9. Top 13 counties where Stockton residents work, not including SJC - percent of overall jobs for Stockton workers age 29 or younger located in workplace. Data from LODES, 2017.*

```{r}
mapview(stockton_lodes_w_top_counties[-c(1,2),], zcol = "Percent of jobs with earnings $1250/month or less", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Percent of jobs with earnings $1250/month or less')
```
*Figure 10. Top 13 counties where Stockton residents work, not including SJC - percent of overall jobs for Stockton workers with earnings $1250/month or less located in workplace. Data from LODES, 2017.*

```{r}
mapview(stockton_lodes_w_top_counties[-c(1,2),], zcol = "Percent of jobs with earnings greater than $3333/month", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Percent of jobs with earnings greater than $3333/month')
```
*Figure 11. Top 13 counties where Stockton residents work, not including SJC - percent of overall jobs for Stockton residents with earnings greater than $3333/month located in workplace. Data from LODES, 2017.*

```{r}
mapview(stockton_lodes_w_top_counties[-c(1,2),], zcol = "Percent of jobs in Goods Producing industry sectors", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Percent of jobs in Goods Producing industry sectors')
```
*Figure 12. Top 13 counties where Stockton residents work, not including SJC - percent of jobs for Stockton residents in Goods Producing industry sectors located in workplace. Data from LODES, 2017.*

```{r}
mapview(stockton_lodes_w_top_counties[-c(1,2),], zcol = "Percent of jobs in Trade, Transportation, and Utilities industry sectors", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Percent of jobs in Trade, Transportation, and Utilities industry sectors')
```
*Figure 13. Top 13 counties where Stockton residents work, not including SJC - percent of jobs for Stockton residents in Trade, Transportation, and Utilities industry sectors located in workplace. Data from LODES, 2017.*

```{r}
mapview(stockton_lodes_w_top_counties[-c(1,2),], zcol = "Percent of jobs in All Other Services industry sectors", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Percent of jobs in All Other Services industry sectors')
```
*Figure 14. Top 13 counties where Stockton residents work, not including SJC - percent of jobs for Stockton residents in All Other Services industry sectors located in workplace. Data from LODES, 2017.*
...
```
*Table 14. Percent of Stockton employed residents working in Stockton, by NAICS industry and wage tier. Data from LODES, 2017.*
...
mapview(stockton_lodes_w_construction_high[1:10,], zcol = "Number of High-wage Construction jobs", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Number of High-wage Construction jobs')
```
*Figure 15. Top 10 counties where Stockton residents work, not including jobs in Stockton - number of high-wage jobs in Construction. Amount in Stockton: 900. Data from LODES, 2017.*
...
mapview(stockton_lodes_w_scientific_high[1:10,], zcol = "Number of Professional, Scientific, and Technical Services jobs", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Number of Professional, Scientific, and Technical Services jobs')
```
*Figure 16. Top 10 counties where Stockton residents work, not including jobs in Stockton - number of high-wage jobs in Professional, Scientific, and Technical Services. Amount in Stockton: 400. Data from LODES, 2017.*
...
```{r}
get_county_vmt <- function(county,year){
  ...
  acs_vars <-
    listCensusMetadata(
      name = paste0(year,"/acs/acs5"),
      type = "variables"
    )

  save(acs_vars,file=paste0("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/acs5_vars_",year,".Rdata"))
  ...
  county_lodes_h_mode <-
    ...
    mutate(
      perc_vehicle =
        1:nrow(county_lodes_h_mode) %>%
        map_dbl(function(x){
          mode_drive <-
            travel_time_mode %>%
            filter(tract == county_lodes_h_mode$residence[x]) %>%
            filter(time == county_lodes_h_mode$tier[x])
  ...
  save(county_lodes_h_route,county_lodes_h_filter,county_lodes_h_mode, file = paste0("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/county_lodes_",county,"_",year,"_tract.Rdata"))
...
# county_commute_vmt <- NULL
# 
for(year in 2015:2016){
  for(county in c("067","099")){
    county_commute_vmt <-
      rbind(
        county_commute_vmt,
        get_county_vmt(county,year)
      )

    save(county_commute_vmt,file = "C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/county_commute_vmt.Rdata")
  }
}

load("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/county_commute_vmt.Rdata")

#2014 appears to be an outlier, remove
#remove other counties
county_commute_vmt <-
  county_commute_vmt %>% 
  filter(Year != 2014) %>% 
  filter(County %in% c("077","001"))
...
```{r}
get_stockton_vmt_bg <- function(year){
  ...
  travel_time_mode <-
    ...
    filter(bg %in% stockton_bgs_full$GEOID) %>% # This importantly filters to only block groups that show up in our Stockton LODES data, which reduces data size considerably.
...
get_stockton_vmt_tract <- function(year){
  ...
  stockton_lodes_h <-
    ca_lodes[which(ca_lodes$h_tract %in% stockton_tracts_full$GEOID),]
  
  stockton_lodes_h_origin_centroids <-
    st_centroid(ca_tracts[which(ca_tracts$GEOID %in% stockton_lodes_h$h_tract),])
  
  stockton_lodes_h_dest_centroids <-
    st_centroid(ca_tracts[which(ca_tracts$GEOID %in% stockton_lodes_h$w_tract),])
  ...
  route <-
    ...
    map_dfr(function(row){
      print(row)
      route <- osrmRoute(
        src = stockton_lodes_h_origin_centroids[which(stockton_lodes_h_origin_centroids$GEOID %in% stockton_lodes_h[row,"h_tract"]),],
        dst = stockton_lodes_h_dest_centroids[which(stockton_lodes_h_dest_centroids$GEOID %in% stockton_lodes_h[row,"w_tract"]),],
        overview = FALSE
      ) %>%
        as.list() %>%
        as.data.frame()
  ...
  save(stockton_lodes_h_route,stockton_lodes_h_filter, file = paste0("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/stockton_lodes_",year,"_tract.Rdata"))
  ...
  travel_time_mode <-
    ...
    filter(tract %in% stockton_tracts_full$GEOID) %>% # This importantly filters to only tracts that show up in our Stockton LODES data, which reduces data size considerably.
    select_if(!names(.) %in% c("GEO_ID","state","county","NAME")) %>%
    dplyr::select(-c(contains("EA"),contains("MA"),contains("M"))) %>%
    gather(
      key = "variable",
    ...
    mutate(
      perc_vehicle =
        ...
        map_dbl(function(x){
          mode_drive <-
            travel_time_mode %>%
            filter(tract == stockton_lodes_h_mode$residence[x]) %>%
            filter(time == stockton_lodes_h_mode$tier[x])
  ...
  save(stockton_lodes_h_route,stockton_lodes_h_filter,stockton_lodes_h_mode, file = paste0("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/stockton_lodes_",year,"_tract.Rdata"))
...
stockton_commute_avg <-
  stockton_lodes_h_filter %>% 
  mutate(
    w_bg = substr(w_tract,1,nchar(w_tract)-1)
  )
  group_by(w_bg) %>% 
  summarize(
    `Average Commute Time, Hours` = mean(duration, na.rm=T)
  ) %>% 
  left_join(ca_tracts, by = c("w_bg" = "GEOID")) %>%
  st_as_sf()

mapview(stockton_commute_avg, zcol = "Average Commute Time, Hours", map.types = c("OpenStreetMap"), legend = TRUE, layer.name = 'Average Commute Time, Hours')
```
*Figure 8. Top 15 counties where Stockton residents work - average commute time, in hours. Data from LODES, 2017.*

The graph above, and our subsequent analyses, focuses on counties with average commute times of less than 3 hours, which eliminates the unrealistic commutes while preserving ~94% of reported commutes.
...
for(year in 2013:2015){
  stockton_commute_vmt <-
    rbind(
      stockton_commute_vmt,
      get_stockton_vmt_bg(year)
    )
}

save(stockton_commute_vmt_tract, file = "C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/stockton_commute_vmt_tract.Rdata")
load("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/stockton_commute_vmt_tract.Rdata")

stockton_commute_vmt_norm <-
  stockton_commute_vmt_tract %>% 
  mutate(
...
for(year in 2011:2017){
  load(
    paste0("C:/Users/derek/Google Drive/City Systems/Stockton Green Economy/LODES/stockton_lodes_",year,"_tract.Rdata")
  )
...
get_stockton_vmt <- function(year){
  ...
  travel_time_mode <-
    ...
    filter(tract %in% stockton_lodes_h_filter$h_tract) %>% # This importantly filters to only block groups that show up in our Stockton LODES data, which reduces data size considerably.
    select_if(!names(.) %in% c("GEO_ID","state","county","NAME")) %>%
    dplyr::select(-c(contains("EA"),contains("MA"),contains("M"))) %>%
    gather(
      key = "variable",
