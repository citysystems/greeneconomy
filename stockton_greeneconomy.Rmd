...
Based on the allocation assumptions, it appears that emissions per resident has decreased by 3.8% between 2005-2016, while emissions per job has decreased by 22.7%. These are both good signs, though there are gaps in the GHG inventories. Next, it is useful to drill down into the sub-sectors to reveal deeper insights.
...
stockton_lodes_h_counties_ghg_map <-
  stockton_lodes_h_counties_ghg[1:15,] %>% 
    transmute(
      ...
      `Total Annual GHG (tCO2e)` = prettyNum(round(ghg,0),big.mark=","),
    `Percent Annual GHG` = round(perc_ghg,2),
    `Average Annual GHG/worker (tCO2e)` = round(avg_ghg,2)
      )
...
get_stockton_vmt <- function(year){
  ...
  total_vmt = sum(stockton_lodes_h_mode$vmt, na.rm=T)
...
stockton_commute_vmt_table <-
  ...
  transmute(
    Year = Year,
    `Commute VMT` = prettyNum(vmt, big.mark=",")
  )

datatable(stockton_commute_vmt_table, rownames = FALSE, options = list(pageLength = 7))
```
*Table 16. Stockton Commute VMT from 2010-2017. Data from LODES and ACS 1-yr.*
...
ggplot(
  ...
  labs(
    title = "Stockton Commute VMT, 2013-2017",
    x = "Year",
    y = "Vehicle Miles Traveled, One-Way Trip"
  )
```
*Figure 18. Stockton Commute VMT from 2010-2017. Data from LODES and ACS 1-yr.*
...
We used the [Cal-Adapt Degree Day tool](https://cal-adapt.org/tools/degree-days) to collect HDDs and CDDs for Stockton from 2013 to 2018, and on to 2040, using the average simulation (CanESM2) under the Representative Concentration Pathway (RCP) 4.5 scenario (in which global GHG emissions peak around 2040, then decline). A full forecast of energy usage will also depend on predicted changes in resident and job populations, which will lead to more buildings and more energy used in buildings, and many other factors that are outside the scope of this analysis.
...
The following table shows projections of electricity and gas usage 
